...
async def check_proxy(proxy,session):
    ...
    async with sem:
        try:
            async with await session.get(url='https://www.baidu.com', headers=hd, proxy=proxy, timeout=5) as response:
                # 如果状态码为200，则代理可用
                if response.status_code == 200:
                    print(f"代理 {proxy} 可用")
                    await record(proxy)
                    cnt_findNewIp += 1
                    if cnt_findNewIp>100:
